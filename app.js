...
function term(a) {
  if (a >= 100 && a <= 500) return [20, 3];
  if (a >= 501 && a <= 1000) return [25, 5];
  if (a >= 1001 && a <= 2000) return [30, 7];
  return null;
}

$('employment').onchange = () => {
  const student = $('employment').value === 'Student';
  $('schoolBox').classList.toggle('hidden', !student);
...
$('amount').oninput = () => {
  ...
  $('terms').innerHTML = t
    ? `<b>Your loan terms:</b> ${t[0]}% interest for ${t[1]} days. Total payment: <b>₱${(a * (1 + t[0] / 100)).toFixed(2)}</b>`
...
$('form').onsubmit = async e => {
  ...
  fd.append('mobile_number', $('phone').value);
  fd.append('date_of_birth', $('dob').value);
  fd.append('employment_type', $('employment').value);
  fd.append('school_facebook_url', $('school').value);
  fd.append('monthly_income', $('income').value);
  fd.append('requested_amount', $('amount').value);
  fd.append('loan_purpose', $('purpose').value);

  fd.append('id_front', $('idFront').files[0]);
  fd.append('id_back', $('idBack').files[0]);
  fd.append('signature', $('signature').files[0]);
  fd.append('verification_video', $('video').files[0]);
